...
object BNFTLMTPRMTBL_retail1 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val bt_plan_benefit_1 = read_from_parquet
 ...
 """)
    ...
    val bf_mbr_written_4 = bf_mbr_written_3.filter(
      "CAST(al_prem_amt AS BIGINT)>=0 AND CAST(a_rate AS BIGINT)>=0 AND CAST(p_rate AS BIGINT)>=0 AND CAST(s_rate AS BIGINT)>=0 AND CAST(q_rate AS BIGINT)>=0 AND CAST(z_rate AS BIGINT)>=0 AND CAST(x_rate AS BIGINT)>=0 AND CAST(y_rate AS BIGINT)>=0 AND CAST(w_rate AS BIGINT)>=0"
    )

    bf_mbr_written_4.createOrReplaceTempView("bf_mbr_written")

    val BF_MBR_WRITTEN_BASE = spark.sql(
      "SELECT CONT_NO,CONT_YYMM, SUM(A_RATE) as A_RATE,SUM(AL_PREM_AMT) as AL_PREM_AMT,SUM(AL_CITY_DL_AMT) as AL_CITY_DL_AMT,SUM(P_RATE) as P_RATE,SUM(Q_RATE) as Q_RATE,SUM(S_RATE) as S_RATE,SUM(Z_RATE) as Z_RATE,SUM(X_RATE) as X_RATE,SUM(Y_RATE) as Y_RATE,SUM(W_RATE) as W_RATE FROM BF_MBR_WRITTEN GROUP BY CONT_NO,CONT_YYMM"
    )
    ...
    val Baseview_interr_1_mid = mr_contract
      ...
      .join(
        bf_iv_av_line.select("CONT_NO", "CONT_YYMM", "MBR_NO", "CLS_ID").distinct,
    ...
    val Baseview_interr_1_pre = spark
      .sql(
        """SELECT T.* from (SELECT A.*,ROW_NUMBER() OVER (partition BY POLICYNO,MEMBERSHIPNO ORDER BY UPD_DATE desc) rnk from Baseview_interr_1_mid A) T where rnk=1"""
      )
      .drop("rnk", "UPD_DATE")
    ...
    val Baseview_interr_2_new = Baseview_interr_1_new
    Baseview_interr_2_new.createOrReplaceTempView("Baseview_interr_2_new")

    val totalmembercount_inter =
      Baseview_interr_2_new.select("POLICYNO", "MBR_NO", "FMLYID", "PLANID").distinct()
...
/*
    ...
    /*println("Product_level_view")
    println(Product_level_view.count())*/
    ...
    val final_view_inter = spark
      ...
      .withColumn("PRMEFFECTIVEDATE",
        when($"MMBRRCD".gt($"SETTLEMENTDATE"), $"MMBRRCD").otherwise($"SETTLEMENTDATE"))
...
FROM Bf_Bfftxnxls01_Hist WHERE trim(doc_type) in('AV','IV') group by TRIM(doc_no)""")*/
    ...
    val BNFTLMTPRMTBL_inter = final_view_inter
      .select(
        "CONT_NO",
        "CONT_YYMM",
        "MBR_NO",
    ...
    val BNFTLMTPRMTBL_nonformatted_2 = spark.sql(
      "SELECT a.* , b.post_year , b.post_month FROM (BNFTLMTPRMTBL_nonformatted_1 a LEFT JOIN ( SELECT * FROM ( SELECT * , row_number() " +
        "OVER (PARTITION BY DOC_NO ORDER BY POST_YEAR ASC) rownum FROM ( SELECT * FROM max_bupa_edl.09_10_non_issue_cases UNION ALL " +
        "SELECT * FROM max_bupa_edl.14_15_non_issue_cases UNION ALL SELECT * FROM max_bupa_edl.15_16_non_issue_cases UNION ALL " +
        "SELECT * FROM max_bupa_edl.16_17_non_issue_cases UNION ALL SELECT * FROM max_bupa_edl.17_18_non_issue_cases UNION ALL " +
        "SELECT * FROM max_bupa_edl.18_19_non_issue_cases ) ) WHERE rownum = 1 ) b ON a.doc_no = b.doc_no)"
    )
    ...
    BNFTLMTPRMTBL
      ...
      .mode("overwrite")
      .parquet(
        "s3://max-lumiq-edl-uat/BupaTransformed/BenifitMaster"
      )
  }
}
